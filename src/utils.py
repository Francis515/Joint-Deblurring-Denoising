...
def noise(img, sigma):
    ...
    return img_noise
'''#test code
kernel = kernel_generator(36)
img = io.imread('F://university/course/4th_1/acquisition/prj/src/francis.JPG',as_grey=True)
...
def Img2patch(img, patch_size, flatten=True):
    ...
    patches = np.zeros(shape=(new_shape[0]*new_shape[1]//(patch_size**2),patch_size,patch_size),dtype=np.float32)
